...
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ReferenceLine, Label, Legend } from 'recharts';
...
import BarChartHelp from '../../components/barChartHelp';
        ...
        {initTeams && <div className="fade-in">
                ...
                {teams && teams?.length > 0 && canActive ? <>
                    <BarChartHelp
                        ...
                        itens={[
                            {
                                title: "Pontuação",
                                description: "Para calcular a pontuação (p), é realizado uma média ponderada de todas as principais métricas coletadas – quantidade de alterações no código (alt) e complexidade ciclomática (cc) – dos arquivos.",
                                formula: "p = ((cc.4,5) + (alt.0,5))/5"
                            },
                            {
                                title: "Alterações (alt)",
                                description: "Soma simples das linhas removidas (del) com as adicionadas (add) nos arquivos do repositório.",
                                formula: "alt = add + del"
                            },
                            {
                                title: "Quantidade de métodos (mt)",
                                description: "O número de métodos/funções dentro do repositório.",
                            },
                            {
                                title: "Complexidade ciclomática (cc)",
                                description: "A complexidade ciclomática é uma métrica de software que tem como objetivo mensurar a complexidade do código fonte com base na análise da quantidade de possíveis resultados dentro de um método. Em síntese, toda vez é detectado uma estrutura condicional ou de repetição a complexidade do método tende a subir, mas essa informação pode ser interpretada de mais de uma forma, visto que ter um código complexo alto demais possa significar que há recursos desnecessários em um método.",
                            }
                        ]} />
                    <BarChart
                        className="fade-in"
                        ...
                        barSize={20}
                    >
                        <Legend wrapperStyle={{ position: 'absolute', top: -10, left: 50 }} />
                        ...
                        <Bar dataKey="value" name="pontuação" yAxisId="left" fill={props.primaryColor} />
            ...
            {teams && teams?.length > 0 && canActive && <div className="teams-analytics-grafic">
                <BarChartHelp
                    ...
                    itens={[
                        {
                            title: "Equilíbrio",
                            description: "O equilíbrio do repositório é um valor dado em porcentagem que pode ser calculado pela somatória das pontuações obtidas pelos contribuidores (PC) dividida pela pontuação do repositório (PR), considerando que a pontuação de cada contribuidor (Pn), nessa conta, não pode ultrapassar o valor obtido pela pontuação do repositório (PR) dividida pelo número de contribuidores (n).",
                            formula: "Pn <= PR/n     >>>     PC = P1 + P2... + Pn     >>>     E = PC/Pr"
                        }
                    ]} />
                <BarChart
                    className="fade-in"
                ...
                >
                    ...
                    <Legend wrapperStyle={{ position: 'absolute', top: -10, left: 50 }} />
                    <CartesianGrid strokeDasharray="3 3" />
                    <Bar dataKey="balance" name="equilíbrio" yAxisId="left" fill={props.secundaryColor} unit="%" />
            ...
            {teams && teams?.length > 0 && canActive && <>
                ...
                <div className="table-page-control-div">
                    <button onClick={() => { goToPage(pageMin, "back") }} style={{ color: (page > pageMin) ? props.secundaryColor : props.primaryColor }}><Icons name="double-back" size={25} color="white" /></button>
                    <button onClick={() => { goToPage(page - 1, "back") }} style={{ color: (page > pageMin) ? props.secundaryColor : props.primaryColor }}><Icons name="back" size={25} color="white" /></button>

                    <input min={pageMin} max={pageMax} style={{ backgroundColor: props.secundaryColor }} type="number" value={page} onChange={handleChangePage}></input>

                    <button onClick={() => { goToPage(page + 1, "next") }} style={{ color: (page < pageMax) ? props.secundaryColor : props.primaryColor }}><Icons name="next" size={25} color="white" /></button>
                    <button onClick={() => { goToPage(pageMax, "next") }} style={{ color: (page < pageMax) ? props.secundaryColor : props.primaryColor }}><Icons name="double-next" size={25} color="white" /></button>
                </div>
                <table className="table-commits" style={{ outlineColor: props.primaryColor }}>
                    <thead>
                        <tr>
                            <th style={{ backgroundColor: props.secundaryColor, borderColor: props.primaryColor }}>Nº</th>
                            <th style={{ backgroundColor: props.secundaryColor, borderColor: props.primaryColor }}>Nome</th>
                            <th style={{ backgroundColor: props.secundaryColor, borderColor: props.primaryColor }}>Repositório</th>
                            <th style={{ backgroundColor: props.secundaryColor, borderColor: props.primaryColor }}>Qtd. Alterações</th>
                            <th style={{ backgroundColor: props.secundaryColor, borderColor: props.primaryColor }}>Qtd. Métodos</th>
                            <th style={{ backgroundColor: props.secundaryColor, borderColor: props.primaryColor }}>C. Ciclomatica</th>
                            <th style={{ backgroundColor: props.secundaryColor, borderColor: props.primaryColor }}>Estado</th>
                        </tr>
                    </thead>
                    <tbody>
                        {
                                ...
                                if ((index + 1) > (itemPerPage * (page - 1)) && (index + 1) <= (itemPerPage * (page))) {
                                    return (<tr key={index} className="table-commits-tr">
                                        <td
                                            ...
                                            className="table-commits-tr-td table-commits-tr-td-text table-id">{index + 1}</td>
                                        <td
                                            style={{ borderColor: props.primaryColor }}
                                            className="table-commits-tr-td table-commits-tr-td-text table-alt">{item.name.split("|")[0]}</td>
                                        <td
                                            style={{ borderColor: props.primaryColor }}
                                            className="table-commits-tr-td table-commits-tr-td-text table-alt">{item.repos}</td>
                                        <td
                                            style={{ borderColor: props.primaryColor }}
                                            className="table-commits-tr-td table-commits-tr-td-text table-alt">{item.changes}</td>
                                        <td
                                            style={{ borderColor: props.primaryColor }}
                                            className="table-commits-tr-td table-commits-tr-td-text table-alt">{item.mt}</td>
                                        <td
                                            style={{ borderColor: props.primaryColor }}
                                            className="table-commits-tr-td table-commits-tr-td-text table-alt">{item.cc}</td>
                                        <td
                                            style={{ borderColor: props.primaryColor }}
                                            className="table-commits-tr-td table-commits-tr-td-text table-alt">{item.commitInWeek ? "Ativo" : "Inativo"}</td>
                                    </tr>);
                                } else {
                                    return null;
                                }
